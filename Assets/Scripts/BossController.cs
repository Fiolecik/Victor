    ...
    void Update()
    {
        if(actionCounter > 0)
		{
            ...
            if(actions[currentAction].shouldMove)
			{
                ...
                if(actions[currentAction].moveToPoints && Vector3.Distance(transform.position, actions[currentAction].pointsToMoveTo.position) > .5f)
				{
                    moveDirection = actions[currentAction].pointsToMoveTo.position - transform.position;
                    moveDirection.Normalize();
				}
			}
